...
const Teams = (props) => {
    ...
    <>
      ...
      <div className="bg_image">
        ...
        <div className="header text-center">
          <button className="btn-outline-light" onClick={() => handleShow2()}>create team <i className="fas fa-plus" style={{fontSize:"28px",paddingLeft:'5px'}}></i></button>
        </div>
              ...
              <Card className="card-wrapper cb1 text-center mr-5" key={obj._id} style={{ width: '16rem' ,background:"transparent" }}>
                ...
                <Card.Body className="task-holder">
                  ...
                  <Link to={`/Teamcontent/${id}`} style={{ textDecoration: "none", color: "black" }}>
                    <Card.Title className="card-header" style={{ "borderRadius": "35px" }}>View {obj.teamName}</Card.Title>
                  </Link>
  ...
  <Dropdown.Item style={{color:"red",padding:"10px" ,margin:"5px",fontSize:"large"  }} className="fas fa-trash-alt  mr-5" onClick={() => {
                          if (obj.teamOwner == id) {
...
<Dropdown.Item style={{color:"black", padding:"10px" ,margin:"5px",fontSize:"large" }} className="fas fa-user-plus mr-5" onClick={() => showing3(obj._id,email)}>  Ivite Member</Dropdown.Item>
<Dropdown.Item style={{color:"red" ,padding:"10px" ,margin:"5px",fontSize:"large" ,display:"block"}} className="fas fa-exclamation-circle" onClick={() => {
                          if (window.confirm('Do you want to leave this team?')) {
                            leaveTeam(obj._id)
                          }
                        }
                        }>  Leave Team</Dropdown.Item>
                         </Dropdown.Menu>
</Dropdown>
                ...
                <p>{obj.body}</p> */}
                  ...
                  <Modal show={show2} onHide={handleClose2} animation={false}>
                    ...
                    <Modal.Body>
                      <div className="form-group">
                        <label style={{ "color": "black" ,fontSize:'30px'}}>Team Name</label>
                        <input type="text" className="form-control" value={teamName} onChange={(e) => setTeamName(e.target.value)} name="teamName" />
                      </div>
                      <div className="form-group">
                        <label style={{ "color": "black" ,fontSize:'30px'}} >Description</label>
                        <textarea rows="2" className="form-control" value={teamDiscription} onChange={(e) => setTeamDiscription(e.target.value)} name="teamDescription"></textarea>
                      </div>
                      <div className="form-group">
                  ...
                  <Modal show={show3} onHide={handleClose3} animation={false}>
                    ...
                    <Modal.Body>
                      ...
                      <div className="form-group">
                        <label style={{ "color": "black",fontSize:'30px' }}>User Email</label>
                    ...
                    <Modal.Body>
                      {
                                members.map(item=>{
                                    return(
                                        <div key={item.id} style={{display:"flex",justifyContent:"space-between",fontSize:'30px' }}>
                                        <p>{item.name}</p>
